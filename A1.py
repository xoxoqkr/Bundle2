...
class Rider(object):
    def __init__(self, env, i, platform, speed = 1, capacity = 3, end_t = 120):
        self.name = i
        self.env = env
        self.visited_route = []
        self.speed = speed
        ...
        self.container = []
        env.process(self.OrderSelect(env, platform))


    def updater(self, point_info):
        pass

    def RunProcess(self, env, platform):
            ...
            if len(self.route) > 0:
                ...
                try:
                    ...
                    if point_info[1] == 0:
                        ...
                        print('T {}/ 주문{}의 가게 도착'.format(int(env.now), point_info[0]))
                    else:
                        self.container.remove(point_info[0])
                        print('T {}/ 주문{}의 고객 도착'.format(int(env.now), point_info[0]))
                        print('제거 이름{} : 제거 대상{} -> 현재 보유 중 {}'.format(point_info[0],platform[point_info[0]],self.onhand))
                        input('확인11')
                        for onhand in self.onhand:
                            if onhand.name == point_info[0]:
                                self.onhand.remove(onhand)
                                print('제거됨')
                                input('확인12')
    ...
    def OrderSelect(self, env, platform):
        if len(platform) > 0:
            #print(sorted(platform, key=operator.attrgetter('fee')))
            #input('확인')
            unserved_orders = []
            ...
            print(sorted(unserved_orders_names))
            input('확인')
            #print(sorted(platform, key=operator.attrgetter('fee')))
            #order = sorted(unserved_orders, key=operator.attrgetter('fee'))[0]
            ...
            if len(self.onhand) == 0:
                ...
                self.run_process = env.process(self.RunProcess(self.env, platform))
            else:
...
def RiderGenerator(env, Rider_dict, Platform, Store_dict, speed = 1, end_time = 120, interval = 1, runtime = 1000, gen_num = 10):
    ...
    while env.now <= runtime or rider_num <= gen_num:
        single_rider = Rider(env,rider_num,Platform, speed = speed, end_t = end_time)
...
for store_name in range(store_num):
    ...
    store = Basic.Store(env, Platform, store_name, loc = loc, capacity = 10)
    #env.process(store.StoreRunner(env, Platform, capacity=store.capacity))
    Store_dict[store_name] = store

env.process(RiderGenerator(env, Rider_dict, Platform, Store_dict, speed = rider_speed, end_time = 120, interval = rider_gen_interval, runtime = run_time, gen_num = rider_num))
